...
@section('page_heading', 'Module Management')
...
<div class="space-y-6">
    ...
    <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <form method="GET" action="{{ route('admin.modules.index') }}" class="flex flex-wrap items-center gap-3 flex-1">
            ...
            @if(request('search') || request('course_id'))
                <a href="{{ route('admin.modules.index') }}" class="py-2 px-3 text-xs font-semibold text-slate-500 hover:text-slate-800 underline">Clear</a>
            @endif
        </form>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                        ...
                        <th class="py-3.5 px-4">Module Title</th>
                        <th class="py-3.5 px-4">Course</th>
                        <th class="py-3.5 px-4 text-center">Lessons</th>
                        <th class="py-3.5 px-4">Live Class</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($modules as $module)
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            <td class="py-3.5 px-4 text-center font-bold text-slate-500">{{ $module->order }}</td>
                            <td class="py-3.5 px-4 font-bold text-slate-900">{{ $module->title }}</td>
                            <td class="py-3.5 px-4">
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-sky-50 text-sky-700 border border-sky-200">
                                    {{ $module->course?->title_en ?? '—' }}
                                </span>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-xs text-slate-500">
                                @if($module->live_class_provider)
                                    <span class="font-semibold capitalize text-slate-700">{{ $module->live_class_provider }}</span>
                                    @if($module->live_class_at)
                                        <div class="text-[11px] text-slate-400">{{ $module->live_class_at->format('d M, h:i A') }}</div>
                                    @endif
                                @else
                            ...
                            <td class="py-3.5 px-4 text-right space-x-2">
                                <a href="{{ route('admin.modules.edit', $module) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-sky-100 text-slate-700 hover:text-sky-800 text-xs font-bold transition-all">
                                    Edit
                                </a>
                                <form method="POST" action="{{ route('admin.modules.destroy', $module) }}" class="inline-block" onsubmit="return confirm('Delete this module?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-12 text-center text-slate-400 text-sm">No modules found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if($modules->hasPages())
            <div class="p-4 border-t border-slate-200 bg-slate-50/50">{{ $modules->links() }}</div>
        @endif
    </div>
