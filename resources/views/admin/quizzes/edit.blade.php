...
<div class="max-w-4xl mx-auto space-y-5" x-data="quizBuilder()">
    ...
    <form method="POST" action="{{ route('admin.quizzes.update', $quiz) }}" @submit="syncJSON()" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" required value="{{ old('title', $quiz->title) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Pass Percentage (%) <span class="text-rose-500">*</span></label>
                    <input type="number" name="pass_percentage" min="1" max="100" value="{{ old('pass_percentage', $quiz->pass_percentage) }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Time Limit (Minutes)</label>
                    <input type="number" name="time_limit_minutes" min="1" value="{{ old('time_limit_minutes', $quiz->time_limit_minutes) }}" placeholder="Leave blank for unlimited" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                </div>
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1.5">Instructions</label>
                <textarea name="instructions" rows="2" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 resize-none">{{ old('instructions', $quiz->instructions) }}</textarea>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-8 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-bold text-sm shadow-md">Update Quiz</button>
        </div>
    </form>
</div>
...
function quizBuilder() {
    return {
        questions: @json($quiz->questions->map(function($q) {
            return [
                'question' => $q->question,
                'type'     => $q->type,
                'points'   => $q->points,
